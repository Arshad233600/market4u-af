...
const Messages: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden h-[calc(100vh-140px)] min-h-[500px] flex">
      {/* Sidebar */}
      <div className={`w-full md:w-80 border-l border-gray-200 flex flex-col ${activeChatId ? 'hidden md:flex' : 'flex'}`}>
         {!isOnline && (
         ...
         <div className="p-4 border-b border-gray-100 bg-gray-50/50">
            <h2 className="font-bold text-gray-800 mb-3">پیام‌ها</h2>
            <div className="relative">
               <input type="text" placeholder="جستجو..." className="w-full bg-white border border-gray-200 rounded-xl pr-9 pl-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-brand-500" />
         ...
         <div className="flex-1 overflow-y-auto">
            {conversations.length === 0 ? (
                ...
                <div key={conv.id} onClick={() => setActiveChatId(conv.id)} className={`p-4 flex gap-3 hover:bg-gray-50 cursor-pointer border-b border-gray-50 transition-colors ${activeChatId === conv.id ? 'bg-brand-50 border-r-4 border-r-brand-500' : ''}`}>
                    <div className="w-12 h-12 bg-gray-200 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-gray-600 relative">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between mb-1">
                            <h4 className="font-bold text-sm truncate text-gray-900">{conv.otherUserName}</h4>
      ...
      {/* Chat Area */}
      <div className={`w-full md:flex-1 flex flex-col bg-gray-50/30 ${!activeChatId ? 'hidden md:flex' : 'flex'}`}>
         {activeChatId && activeConversation ? (
            <>
               <div className="p-3 border-b border-gray-200 flex items-center justify-between bg-white z-10 shadow-sm h-[72px]">
                  <div className="flex items-center gap-3">
                     <button onClick={() => setActiveChatId(null)} className="md:hidden text-gray-500 p-2 -mr-2 hover:bg-gray-100 rounded-full"><ArrowRight className="w-6 h-6" /></button>
                  ...
                  <button className="text-gray-400 p-2 hover:bg-gray-100 rounded-full"><MoreVertical className="w-5 h-5" /></button>
               </div>
                  ...
                  {messages.map((msg, idx) => {
                     ...
                     return (
                        <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group animate-in slide-in-from-bottom-2 duration-300`}>
                           <div className={`max-w-[75%] p-3 rounded-2xl text-sm relative shadow-sm ${isMe ? 'bg-brand-600 text-white rounded-br-none' : 'bg-white text-gray-800 border border-gray-200 rounded-bl-none'}`}>
                              ...
                              <div className={`text-[10px] mt-1 flex items-center gap-1 justify-end ${isMe ? 'text-brand-100' : 'text-gray-400'}`}>
               ...
               <div className="p-3 bg-white border-t border-gray-100 flex gap-2 items-end">
                   <button className="p-3 text-gray-400 hover:bg-gray-100 rounded-full transition-colors"><Plus className="w-5 h-5 text-gray-400" /></button>
                   <input 
                      value={newMessage}
                      onChange={(e) => setNewMessage(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()}
                      className="flex-1 bg-gray-100 rounded-2xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-brand-500 transition-shadow"
                   ...
                   {newMessage.trim() ? (
                       ...
                       <button 
                        ...
                        className={`p-3 rounded-full transition-all ${isListening ? 'bg-red-50 text-red-500 animate-pulse border border-red-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8">
               ...
               <p className="font-bold text-gray-500">گفتگویی انتخاب نشده است</p>
